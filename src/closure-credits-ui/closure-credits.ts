(() => {
  type Closure = { id: string; starts_on: string; reopens_on: string; reason: string; status: string };
  type Detail = { invoice: string; plan: string; periodStart: string; periodEnd: string; chargedCents: number; closedDays: number; periodDays: number; creditCents: number };
  type Row = { id: string; account_label: string; customer_id: string | null; state: string; amount_cents: number; fingerprint: string | null; note?: string; applied_at?: string; stripe_transaction_id?: string; preview?: { details: Detail[]; warnings: string[] } };
  type Dependencies = { root: HTMLElement; token: () => string; confirm: (options: { title: string; message: string; confirmLabel: string; cancelLabel: string }) => Promise<boolean> };
  ...
  const stateLabel = (row: Row) => row.state === 'ready' ? (row.amount_cents ? 'Ready' : 'No eligible charge') : ({ pending: 'Not previewed', review: 'Needs review', excluded: 'Excluded', applying: 'Check / retry', applied: 'Credit issued' }[row.state] || row.state);

  async function mount({ root, token, confirm }: Dependencies): Promise<void> {
    ...
    async function api(action: string, data: Record<string, unknown> = {}): Promise<any> {
      const session = token();
      ...
      if (!response.ok) throw new Error(payload.error || 'The request could not complete. Refresh to check progress.');
      return payload;
    }
    async function refresh(): Promise<void> {
      if (closure) {
        const result = await api('get');
        closure = result.closure; rows = result.rows;
      }
    ...
    function draw(): void {
      ...
      root.innerHTML = `
        ...
        `}`;
      root.querySelector<HTMLInputElement>('#closureReviewed')?.addEventListener('change', event => { reviewed = (event.target as HTMLInputElement).checked; draw(); });
      root.querySelector<HTMLFormElement>('#closureForm')?.addEventListener('submit', event => {
      ...
      root.querySelectorAll<HTMLButtonElement>('[data-action]').forEach(button => button.addEventListener('click', () => void handle(button)));
    }
    async function run(task: () => Promise<void>): Promise<void> {
      if (busy) return;
      busy = true; errors = []; message = 'Working…'; draw();
      try { await task(); }
      catch (e) { errors.push((e as Error).message); }
      finally { busy = false; message = ''; draw(); }
    }
    async function processRows(action: 'preview' | 'apply'): Promise<void> {
      ...
      for (const row of candidates) {
        ...
        message = `${action === 'preview' ? 'Previewing' : 'Applying'} ${done + 1} of ${candidates.length}: ${row.account_label}`; draw();
        try { await api(action, { row_id: row.id }); }
        catch (e) { errors.push(`${row.account_label}: ${(e as Error).message}`); }
        done++;
      }
      await refresh();
    }
    async function handle(button: HTMLButtonElement): Promise<void> {
      const action = button.dataset.action;
      if (busy) return;
      if (action === 'back') { closure = null; rows = []; reviewed = false; errors = []; draw(); return; }
      if (action === 'apply') {
        const candidates = rows.filter(r => r.state === 'ready' && r.amount_cents > 0);
        if (!await confirm({ title: 'Apply closure credits?', message: `Issue ${money(candidates.reduce((n, r) => n + r.amount_cents, 0))} in Stripe credits to ${candidates.length} accounts? Credits reduce each account’s next finalized invoice. This issues real credits and locks this preview.`, confirmLabel: 'Apply credits', cancelLabel: 'Keep preview' })) return;
      }
      if (action === 'cancel' && !await confirm({ title: 'Cancel this draft?', message: 'This releases the dates so you can create a corrected preview. No credits will be issued.', confirmLabel: 'Cancel draft', cancelLabel: 'Keep draft' })) return;
      const exclusion = action === 'exclude' ? root.querySelector<HTMLInputElement>(`[data-exclusion="${button.dataset.row}"]`)?.value.trim() : '';
      if (action === 'exclude' && !exclusion) { errors = ['Enter a reason before excluding an account.']; draw(); return; }
      await run(async () => {
        if (action === 'open') { const result = await api('get', { closure_id: button.dataset.id }); closure = result.closure; rows = result.rows; reviewed = false; }
        else if (action === 'refresh') await refresh();
        else if (action === 'preview') { reviewed = false; await processRows('preview'); }
        else if (action === 'apply') { await api('begin', { reviewed_refunds: reviewed, expected_rows: rows.map(r => ({ id: r.id, state: r.state, fingerprint: r.fingerprint, amount_cents: r.amount_cents })).sort((a, b) => a.id.localeCompare(b.id)) }); await refresh(); await processRows('apply'); }
        else if (action === 'resume') await processRows('apply');
        else if (action === 'exclude') { await api('exclude', { row_id: button.dataset.row, note: exclusion }); reviewed = false; await refresh(); }
        else if (action === 'cancel') { await api('cancel'); closure = null; rows = []; reviewed = false; await refresh(); }
      });
    }
    await run(refresh);
  }
  (window as unknown as { RORC_CLOSURE_CREDITS: { mount: typeof mount } }).RORC_CLOSURE_CREDITS = { mount };
})();
