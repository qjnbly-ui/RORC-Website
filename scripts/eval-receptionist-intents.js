...
async function main() {
  ...
  for (const item of fixture) {
    ...
    results.push({ item, actual, correct: intentCorrect && formCorrect && actionCorrect });
  }
  ...
  for (const result of wrong) {
    console.error(`MISS: ${result.item.phrase}\n  expected ${result.item.intent}; received ${result.actual.intent} (${result.actual.confidence})`);
  }
