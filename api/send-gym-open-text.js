...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const toNumbers = parseSmsDestinations(req.body?.to, GYM_OPEN_TO_NUMBER);
    ...
    const results = await Promise.all(toNumbers.map(async (toNumber) => {
      ...
      if (!response.ok) {
        throw new Error(`${toNumber}: ${result?.message || "Twilio SMS request failed."}`);
      }
      return result;
    }));
    ...
    return res.status(200).json({
      success: true,
      sent: results.length,
      sids: results.map((result) => result.sid).filter(Boolean)
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
