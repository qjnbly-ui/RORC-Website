...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const heaterPinRaw = req.body?.heaterPin;
    const heaterPin = heaterPinRaw == null ? null : String(heaterPinRaw).trim();
    ...
    const isManager = actorRole === "Account Manager";
    if (!isManager && actor.account_id !== accountId) {
      return res.status(403).json({ success: false, error: "You can only edit your shared account heater PIN." });
    }
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/accounts?id=eq.${encodeURIComponent(accountId)}`, {
      ...
      body: JSON.stringify({
        heater_pin: heaterPin || null
      })
    });

    if (!response.ok) {
      const text = await response.text();
      throw new Error(`Could not update account heater PIN: ${response.status} ${text}`);
    }
