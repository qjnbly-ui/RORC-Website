...
GPT-OSS router and answer requests use low reasoning effort and a completion budget that leaves room for both reasoning tokens and the final response. An explicitly truncated completion is rejected and retried instead of being spoken. Repeat requests replay the most recent answer deterministically, casual greetings do not volunteer unrelated live facility readings, and provider failures return only information relevant to the routed request.
